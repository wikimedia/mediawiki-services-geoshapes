...
const { mwApiPost } = require('./api-util');
 ...
 */
class GeoShapes {
  constructor(type, reqParams, app) {
    this.app = app;
    ...
    if (reqParams.ids) {
      ...
      if (this.ids.length > this.config.maxidcount) {
        throw new Err('No more than %d IDs is allowed', this.config.maxidcount);
      }
  ...
  _expandProperties() {
    ...
    }, this.config.mwapiHeaders).then((apiResult) => {
